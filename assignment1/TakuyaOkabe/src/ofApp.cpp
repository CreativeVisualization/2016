...
//--------------------------------------------------------------
void ofApp::update(){
    location = location + velocity; //速度から位置を更新
}
...
void ofApp::draw(){
    ...
    ofDrawRectangle(location, 40, 40); //半径40の円を描画
    
    //画面の端でバウンドするように
    if (location.x < 0 || location.x > ofGetWidth() - 40) { //画面の左右ではみ出したら
        velocity.x *= -1; //横向きの速度を反転(バウンド)
    }
    if (location.y < 0 || location.y > ofGetHeight() - 40) { //画面の左右ではみ出したら
