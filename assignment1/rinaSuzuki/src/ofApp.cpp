...
void ofApp::setup(){
    // 画面基本設定
    ofSetFrameRate(300); //秒間60コマで描画
 
    ofSetBackgroundColor(0, 128, 0);   //背景色を黒に
    
    //位置と速度を初期化
    location = ofVec2f(ofGetWidth()/2, ofGetHeight()/2); //画面の中心に
    velocity = ofVec2f(ofRandom(-50, 50), ofRandom(-10, 10)); //ランダムな速度で
}
...
void ofApp::draw(){
    //計算した位置に円を描画
    ofSetColor(255, 0, 0); //円の色
    
    ofDrawCircle(location, 250); //半径40の円を描画
